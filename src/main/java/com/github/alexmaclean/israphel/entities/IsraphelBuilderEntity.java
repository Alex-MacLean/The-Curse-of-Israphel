...
public class IsraphelBuilderEntity extends AbstractIsraphelEntity {
    private int placeTimer = 1000;
    private int lifeTimer = 5100;

    public IsraphelBuilderEntity(World worldIn) {
        super(worldIn);
    ...
    public void initEntityAI() {
        ...
        this.tasks.addTask(7, new EntityAIWatchClosest(this, AbstractIllager.class, 8.0f));
        ...
        if (this.placeTimer == 0 && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.world, this)) {
            ...
            if (!world.isRemote && Config.BuilderBuilds) {
                if (blockAt.getBlock() == Blocks.GRASS.getDefaultState().getBlock() && y >= 62 || blockAt.getBlock() == Blocks.DIRT.getDefaultState().getBlock() && y >= 62) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "dirt_structure"));
                ...
                if (blockAt.getBlock() == Blocks.STONE.getBlockState().getBlock() && y <= 61) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "ominous_mineshaft"));
                ...
                if (blockAt.getBlock() == Blocks.SAND.getDefaultState().getBlock() && y >= 62) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "sand_structure"));
                    template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false));
                    this.swingArm(EnumHand.MAIN_HAND);
                }
                if (blockAt.getBlock() == Blocks.SAND.getStateFromMeta(1).getBlock() && y >= 62) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "red_sand_structure"));
                    template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false));
                    this.swingArm(EnumHand.MAIN_HAND);
                }
                if (blockAt.getBlock() == Blocks.HARDENED_CLAY.getBlockState().getBlock() && y >= 62) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "terracotta_structure"));
                ...
                if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock() && y >= 62) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "stone_structure"));
                    template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false));
                    this.swingArm(EnumHand.MAIN_HAND);
                }
                if (blockAt.getBlock() == Blocks.NETHERRACK.getDefaultState().getBlock()) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "netherrack_structure"));
                    template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false));
                    this.swingArm(EnumHand.MAIN_HAND);
                }
                if (blockAt.getBlock() == Blocks.END_STONE.getDefaultState().getBlock()) {
                    Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation(Israphel.MODID, "end_stone_structure"));
                    template.addBlocksToWorldChunk(world, new BlockPos(x, y, z), new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setIgnoreEntities(false));
                    this.swingArm(EnumHand.MAIN_HAND);
                }
            }
